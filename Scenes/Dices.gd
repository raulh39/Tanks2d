...
export (float) var child_dices_position_y = 200
export (int,1,10) var max_number_of_dices = 5

var _dice_scene := preload("res://Scenes/Dice.tscn")
var _child_dices := []
var _next_child_position_x := 100
var _numbers_rolled := []

func _ready():
	_on_AddButton_pressed() #We want at least one dice
...
func _on_AddButton_pressed():
	...
	new_dice.position = Vector2(_next_child_position_x, child_dices_position_y)
	_next_child_position_x += dice_margin + new_dice.get_size().x
